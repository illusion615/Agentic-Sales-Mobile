 ...
 * never leaves `isLoading`, so any UI gated on it spins forever. Racing the
 * call against a timeout converts that silent hang into a normal rejection, so
 * react-query can settle into an error state and later refetch on reconnect.
 */
