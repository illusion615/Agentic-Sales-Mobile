...
export async function runFrame(ctx: FrameRunContext): Promise<FrameRunOutcome> {
  ...
  if (ctx.conversationStateText && ctx.conversationStateText.trim()) {
    contextBlock += renderPrompt('frame.conversationState', {
      state: ctx.conversationStateText.trim(),
    });
  }
  if (tail.length > 0) {
    const turns = tail.map((m) => `${m.role}: ${m.content}`).join('\n');
    contextBlock += renderPrompt('frame.conversationContext', { turns });
  }

  const system = renderPrompt('frame.classify', {
    pageContext: pageBlock,
  ...
  const frameVariables = {
    pageContext: pageBlock,
    conversationContext: contextBlock,
    // `__user` is metadata, not a placeholder: it carries the user turn so a
    // replay can reproduce the whole call, not just the system prompt.
    __user: ctx.userMessage,
  };

  let resp = await invokeFlowForLLM({
    messages,
    responseFormat: 'text',
  }, { label: 'Frame', prompt: { key: 'frame.classify' } });
  ...
  if (!parsed) {
    ...
    resp = await invokeFlowForLLM({
      messages,
      responseFormat: 'text',
    }, { label: 'Frame · retry', prompt: { key: 'frame.classify' } });
