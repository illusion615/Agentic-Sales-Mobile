...
/**
 * Extract structured visit data from text using Copilot Studio Direct Line.
 * This helper manages the Direct Line conversation lifecycle for extraction.
 */
export async function extractVisitDataFromText(
  ...
  copilotConversationRef: React.MutableRefObject<ConversationInfo | null>,
  watermarkRef: React.MutableRefObject<string | undefined>
  ...
  try {
    ...
    await sendCopilotMessage(conversation, userId || 'anonymous', extractionPrompt);
    ...
    while (attempts < maxAttempts) {
      ...
      const { activities, watermark: newWatermark } = await pollMessages(conversation, watermarkRef.current);
