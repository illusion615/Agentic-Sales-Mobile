...
import type { SalesPromptKey } from '@/prompts';
...
export interface FunctionDefinition {
  ...
  llmBacked?: boolean;
  /** Catalogued system prompt (src/prompts). The executor renders it through the registry. */
...
export const availableFunctions: FunctionDefinition[] = [
  ...
  // ===== LLM-backed AI Skills (page-level AI, also callable from dialog) =====
  {
    ...
    promptKey: 'skill.generateInsight',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    promptKey: 'skill.generateBriefTranscript',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    promptKey: 'skill.summarizeEntities',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    responseFormat: 'text',
    outputSchema: z.string().min(1),
    promptKey: 'skill.generateEntitySummary',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    promptKey: 'skill.generateEntityInsight',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    promptKey: 'skill.analyzeOpportunity',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    responseFormat: 'text',
    outputSchema: z.string().min(1),
    promptKey: 'skill.summarizeDAGResults',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    outputSchema: z.object({
      ...
      chart: z.object({
        type: z.enum(['bar', 'donut', 'line']),
        dimension: z.string(),
        metric: z.enum(['amount', 'count']),
        title: z.string().optional(),
      }).optional(),
    }).passthrough(),
    promptKey: 'skill.analyzeResults',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    responseFormat: 'text',
    outputSchema: z.string().min(1),
    promptKey: 'skill.generateVoiceSummary',
    parameters: {
      type: 'object',
      properties: {
