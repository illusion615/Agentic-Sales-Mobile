...
const TONE: Record<SectionStatus, { dot: string; text: string; ring: string }> = {
  ...
  'needs-review': { dot: 'bg-blue-500', text: 'text-blue-700', ring: 'ring-blue-200' },
  settled: { dot: 'bg-emerald-500', text: 'text-emerald-600', ring: 'ring-transparent' },
  blank: { dot: 'bg-slate-300', text: 'text-slate-400', ring: 'ring-transparent' },
};
...
export function ReviewSectionCard({
  ...
  return (
    <section
      ...
      className={`overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ${tone.ring} scroll-mt-24`}
    >
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-baseline gap-2">
            <span className="truncate font-medium text-slate-900">{review.section.title}</span>
            <span className={`shrink-0 text-xs ${tone.text}`}>{review.headline}</span>
          </span>
          {/* A closed section still has to say what is inside it. */}
          {!open && review.digest && (
            <span className="mt-0.5 block truncate text-xs text-slate-400">{review.digest}</span>
          )}
        </span>
          ...
          <div className="flex flex-col gap-4 border-t border-slate-100 p-4">
            {review.unconfirmed.length > 1 && (
              <button
                ...
                className="self-start rounded-full bg-blue-600 px-3 py-1 text-xs text-white"
              >
